/*	Delaunay Triangulation :
	Initialize the coordinate range with trig::LOTS.
	trig::find : returns the triangle that contains the point.
	trig::add_point : adds the point to the triangulation.
	Some tri is in the triangulation if tri::has_child () == 0.
	To find the neighbouring tris of u, check u.e[i].tri, with vertice of the corresponding edge u.p[(i + 1) % 3] and u.p[(i + 2) % 3]. */
