/*	Online half plane intersection : complexity $\color{commentcolor}O(n)$ each operation. */
std::vector <point> cut (const std::vector<point> &c, line p) {
	...
	return ret; }
/*	Offline half plane intersection : complexity $\color{commentcolor}O(n\log n)$. */
