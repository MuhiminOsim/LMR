/*	Dominator tree : finds the immediate dominator (idom[]) of each node, idom[x] will be x if x does not have a dominator, and will be -1 if x is not reachable from s. */
template <int MAXN = 100000, int MAXM = 100000>
struct dominator_tree {
	using edge_list = std::vector <int> [MAXN];
	int dfn[MAXN], sdom[MAXN], idom[MAXN], id[MAXN], f[MAXN], fa[MAXN], smin[MAXN], stamp;
	void predfs (int x, const edge_list &succ) {
		id[dfn[x] = stamp++] = x;
		for (int y : succ[x]) {
			if (dfn[y] < 0) { f[y] = x; predfs (y, succ); } } }
	int getfa (int x) {
		if (fa[x] == x) return x;
		int ret = getfa (fa[x]);
		if (dfn[sdom[smin[fa[x]]]] < dfn[sdom[smin[x]]]) smin[x] = smin[fa[x]];
		return fa[x] = ret; }
	void solve (int s, int n, const edge_list &succ) {
		std::fill (dfn, dfn + n, -1); std::fill (idom, idom + n, -1);
		static edge_list pred; static std::queue <int> tmp[MAXN];
		std::fill (pred, pred + n, std::vector <int> ());
		for (int i = 0; i < n; ++i) for (int j = 0; j < succ[i].size (); ++j)
			pred[succ[i][j]].push_back (i);
		stamp = 0; std::fill (tmp, tmp + n, std::queue <int> ()); predfs (s, succ);
		for (int i = 0; i < stamp; ++i) fa[id[i]] = smin[id[i]] = id[i];
		for (int o = stamp - 1; o >= 0; --o) {
			int x = id[o];
			if (o) {
				sdom[x] = f[x];
				for (int p : pred[x]) {
					if (dfn[p] < 0) continue;
					if (dfn[p] > dfn[x]) { getfa (p); p = sdom[smin[p]]; }
					if (dfn[sdom[x]] > dfn[p]) sdom[x] = p; }
				tmp[sdom[x]].push (x); }
			while (!tmp[x].empty ()) {
				int y = tmp[x].front (); tmp[x].pop (); getfa (y);
				if (x != sdom[smin[y]]) idom[y] = smin[y];
				else idom[y] = x; }
			for (int v : succ[x]) if (f[v] == x) fa[v] = x; }
		idom[s] = s; for (int i = 1; i < stamp; ++i) {
			int x = id[i]; if (idom[x] != sdom[x]) idom[x] = idom[idom[x]]; } } };

