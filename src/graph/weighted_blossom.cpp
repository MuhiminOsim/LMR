/*	Weighted blossom algorithm (vfleaking ver.) : maximum matching for general weighted graphs with complexity $\color{commentcolor}O(n^3)$.
Usage : Set n to the size of the vertices. Run init (). Set g[][].w to the weight of the edge. Run solve ().
The first result is the answer, the second one is the number of matching pairs. Obtain the matching with match[].
Note : 1-based. */
