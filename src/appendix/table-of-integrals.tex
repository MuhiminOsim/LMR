...
\paragraph{$\sqrt{\pm ax^2+bx+c}$ ($a>0$)}
...
\paragraph{$\sqrt{\pm\frac{x-a}{x-b}}$ \& $\sqrt{(x-a)(x-b)}$}
...
\paragraph{Inverse triangular function ($a>0$)}
