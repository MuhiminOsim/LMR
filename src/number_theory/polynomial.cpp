...
struct polynomial {
	ntt <MAXN> tr;
	/*	inverse : finds a polynomial $\color{commentcolor}b$ so that $\color{commentcolor}a(x)b(x)\equiv 1\mod x^n\mod mod$. 
Note : $\color{commentcolor}n$ must be a power of $\color{commentcolor}2$. 2x max length. */
	...
	/*	sqrt : finds a polynomial $\color{commentcolor}b$ so that $\color{commentcolor}b^2(x)\equiv a(x)\mod x^n\mod mod$. 
Note : $\color{commentcolor}n\geq 2$ must be a power of $\color{commentcolor}2$. 2x max length. */
	void sqrt (int *a, int *b, int n, int mod, int prt) {
		...
		for (int m = 2; m <= n; m <<= 1) {
			...
			tr.solve (b, m + m, 1, mod, prt); std::fill (b + m, b + m + m, 0); } }
	/*	divide : given polynomial $\color{commentcolor}a(x)$ and $\color{commentcolor}b(x)$ with degree $\color{commentcolor}n$ and $\color{commentcolor}m$ respectively, finds $\color{commentcolor}a(x)=d(x)b(x)+r(x)$ with $\color{commentcolor}deg(d)\leq n-m$ and $\color{commentcolor}deg(r)<m$. 4x max length required. */
